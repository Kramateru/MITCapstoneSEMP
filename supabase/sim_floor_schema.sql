...
create table if not exists public.profiles (
  ...
  avatar_url text,
  status text not null default 'available' check (status in ('available', 'busy', 'offline', 'on_call')),
...
'Storage path conventions: recordings/{trainee_id}/{scenario_id}/{attempt_id}/{timestamp}.wav for attempts and assets/{trainer_id}/{scenario_id_or_draft}/{asset_kind}/{timestamp}_{filename} for trainer-managed member, ringer, and hold audio.';
