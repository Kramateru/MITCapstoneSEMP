...
create table if not exists public.call_simulation_event (
  id text primary key default gen_random_uuid()::text,
  session_id text not null references public.sim_session(id) on delete cascade,
  scenario_id text references public.scenario(id) on delete set null,
...
create index if not exists idx_call_simulation_event_session
  on public.call_simulation_event (session_id, created_at desc);

create index if not exists idx_call_simulation_event_scenario
  on public.call_simulation_event (scenario_id, created_at desc);

create index if not exists idx_call_simulation_event_user
  on public.call_simulation_event (user_id, created_at desc);

alter table public.session_responses enable row level security;

alter table public.call_simulation_event enable row level security;
...
drop policy if exists "call_simulation_event_select_related_users" on public.call_simulation_event;
create policy "call_simulation_event_select_related_users"
on public.call_simulation_event
for select
using (
  public.is_trainer_or_admin()
  or exists (
    select 1
    from public.sim_session session_row
    where session_row.id = call_simulation_event.session_id
      and session_row.trainee_id = auth.uid()::text
  )
);

drop policy if exists "call_simulation_event_insert_related_users" on public.call_simulation_event;
create policy "call_simulation_event_insert_related_users"
on public.call_simulation_event
for insert
with check (
  ...
  and exists (
    ...
    where session_row.id = call_simulation_event.session_id
      and (
        session_row.trainee_id = auth.uid()::text
