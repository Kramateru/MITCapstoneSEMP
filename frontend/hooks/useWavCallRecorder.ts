...
async function encodeMp3(samples: Float32Array, sampleRate: number) {
  ...
  const Mp3Encoder = (mod as any).Mp3Encoder
...
export function useWavCallRecorder(options?: { onLevel?: (level: number) => void }) {
  ...
  const startCapture = useCallback(async () => {
    ...
    try {
      ...
      microphoneGain.connect(mixGain);
  ...
  const setMicrophoneMuted = useCallback((muted: boolean) => {
    ...
    if (!audioContext || !microphoneGain) {
      return;
    }

    try {
      microphoneGain.gain.setTargetAtTime(muted ? 0 : 1, audioContext.currentTime, 0.015);
    } catch {
      microphoneGain.gain.value = muted ? 0 : 1;
    }
  }, []);
  ...
  const stopCapture = useCallback(async () => {
    ...
    try {
      const blob = await encodeMp3(mergedSamples, sampleRateRef.current);
