...
export default function TraineeDashboard() {
  ...
  const loadTraineeStats = useCallback(async () => {
    try {
      const statsData = await apiFetch<TraineeStats>('/api/trainee/stats');
      setStats(statsData);
    } catch (error) {
      console.error('Error loading trainee stats:', error);
    }
  }, []);

  const loadTraineeSessions = useCallback(async () => {
    try {
      const sessionsData = await apiFetch<{ sessions?: PracticeSession[] }>('/api/trainee/sessions');
      setSessions(sessionsData.sessions || []);
    } catch (error) {
      console.error('Error loading trainee sessions:', error);
    }
  }, []);

  const loadTraineeCoachingLogs = useCallback(async () => {
    try {
      const coachingData = await apiFetch<{ logs?: CoachingLogSummary[] }>('/api/certification/coaching/logs');
  ...
  const loadSimFloorWorkspace = useCallback(async () => {
    try {
      ...
      const reportPayload = await apiFetch<SimFloorDashboardReport>(`/api/call-simulation/reports/trainee/${user.user_id}`);
      setSimFloorReport(reportPayload);

      const availablePayload = await apiFetch<{ scenarios?: SimFloorAssignedScenario[] }>('/api/call-simulation/available');
      setAssignedSimFloorScenarios(availablePayload.scenarios || []);
    } catch (error) {
  ...
  const refreshDashboard = useCallback(async () => {
    ...
    try {
      await Promise.all([
        loadSimFloorWorkspace(),
        loadTraineeStats(),
        loadTraineeSessions(),
        loadTraineeCoachingLogs(),
      ]);
    } finally {
      setLoadingWorkspace(false);
      hasLoadedWorkspace.current = true;
    }
  }, [loadSimFloorWorkspace, loadTraineeStats, loadTraineeSessions, loadTraineeCoachingLogs]);
            ...
            <div className="space-y-4">
              ...
              <form
                ...
                onSubmit={async (e) => {
                  ...
                  setIsChanging(true);
                  try {
                    await apiFetch<void>('/api/users/change-password', {
                      method: 'POST',
                      body: JSON.stringify({
              ...
              >
                ...
                <div className="dashboard-form-actions md:col-span-3">
                  <Button
                    type="submit"
                    disabled={
                      isChanging ||
          ...
          {stats ? (
            <div className="dashboard-metrics-grid">
              <MetricCard
                ...
                icon={<LazyIcon name="Play" className="size-5" />}
                tone="blue"
              />
              <MetricCard
                label="Average Score"
                ...
                icon={<LazyIcon name="Target" className="size-5" />}
                tone="green"
              />
              <MetricCard
                ...
                icon={<LazyIcon name="Medal" className="size-5" />}
                tone="violet"
              />
              <MetricCard
                label="Recorded Time"
                ...
                icon={<LazyIcon name="Clock" className="size-5" />}
                tone="amber"
              />
              <MetricCard
                label="Certificates"
                ...
                icon={<LazyIcon name="GraduationCap" className="size-5" />}
                tone="green"
              />
            </div>
          ) : null}
              ...
              >
                <div className="dashboard-actions-grid">
                  <ActionCard
                    ...
                    icon={<LazyIcon name="BookOpen" className="size-5" />}
                    tone="blue"
                  />
                  <ActionCard
                    ...
                    icon={<LazyIcon name="ClipboardList" className="size-5" />}
                    tone="green"
                  />
                  <ActionCard
                    href={simFloorHref}
                    ...
                    icon={<LazyIcon name="Mic" className="size-5" />}
                    tone="amber"
                  />
                  <ActionCard
                    ...
                    icon={<LazyIcon name="MessageSquare" className="size-5" />}
                    tone="amber"
                  />
                  <ActionCard
                    ...
                    icon={<LazyIcon name="TrendingUp" className="size-5" />}
                    tone="violet"
