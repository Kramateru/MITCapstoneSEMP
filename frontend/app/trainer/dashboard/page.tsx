  ...
  const fetchTrainerData = useEffectEvent(async () => {
    try {
      ...
      const [
        ...
        apiFetch<TrainerStats>('/api/trainer/stats', { cache: 'no-store' }),
        apiFetch<TrainerLearningInsightsResponse>('/api/analytics/trainer/learning-insights', { cache: 'no-store' }),
        apiFetch<CoachingHubResponse>('/api/certification/coaching/hub', { cache: 'no-store' }),
        apiFetch<CoachingCompliance>('/api/certification/coaching/compliance', { cache: 'no-store' }),
        apiFetch<{ batches: BatchItem[] }>('/api/trainer/batches', { cache: 'no-store' }),
      ]);
      ...
      if (insightsResult.status === 'fulfilled') {
        for (const row of insightsResult.value.batch_comparison || []) {
          batchMetricsById.set(row.batch_id, row);
        }
      }

      if (coachingHubResult.status === 'fulfilled') {
        const coachingHubData = coachingHubResult.value;
      ...
      if (coachingResult.status === 'fulfilled') {
        setCoachingStats(coachingResult.value);
      }
      ...
      const lastError = [statsResult, insightsResult, coachingHubResult, coachingResult, batchesResult].find(
        (result) => result.status === 'rejected',
      ) as PromiseRejectedResult | undefined;

      if (lastError) {
        console.warn('Partial trainer dashboard load error:', lastError.reason);
      }
    } catch (fetchError) {
    ...
    <DashboardLayout sidebarItems={sidebarItems} userRole="trainer">
      <div className="space-y-4 lg:space-y-4">
        <DashboardHero
          ...
          actions={
            <div className="action-row w-full justify-start xl:justify-end">
        ...
        <div className="dashboard-metrics-grid">
          <MetricCard
            ...
            icon={<LazyIcon name="Users" className="size-5" />}
            tone="blue"
          />
          <MetricCard
            ...
            icon={<LazyIcon name="Layers3" className="size-5" />}
            tone="violet"
          />
          <MetricCard
          ...
          <MetricCard
            ...
            icon={<LazyIcon name="FileText" className="size-5" />}
            tone="blue"
          />
          <MetricCard
            ...
            icon={<LazyIcon name="AlertCircle" className="size-5" />}
            tone="amber"
          />
          <MetricCard
        ...
        <div className="dashboard-balanced-grid">
          ...
          >
            {showSecondaryPanel && loading && !coachingQueue.length ? (
              <EmptyStatePanel
                    ...
                    <div className="mt-5 flex justify-end">
                      <Button asChild variant="outline">
                        <Link href="/trainer/coaching">
                          Review in Coaching
                          <LazyIcon name="ArrowRight" className="size-4" />
                        </Link>
                      </Button>
                    </div>
                  </div>
