...
export default function LoginPage() {
  ...
  useEffect(() => {
    setHasHydrated(true)
    const videoDelayTimer = window.setTimeout(() => {
      setShouldLoadVideo(true)
    }, 180)

    return () => window.clearTimeout(videoDelayTimer)
  }, [])
  ...
  return (
    <main className="relative min-h-screen overflow-x-hidden bg-slate-950 text-white">
      ...
      <div className="relative z-10 flex min-h-screen items-start justify-center px-3 py-3 sm:px-5 sm:py-5 lg:items-center lg:px-6 lg:py-4">
        <div className="mx-auto grid w-full max-w-[76rem] overflow-hidden rounded-[30px] border border-white/16 bg-white/8 shadow-[0_42px_120px_-50px_rgba(0,0,0,0.96)] backdrop-blur-lg lg:min-h-[44rem] lg:grid-cols-[1fr_0.96fr]">
          <section className="relative overflow-hidden bg-[linear-gradient(180deg,rgba(16,50,68,0.78),rgba(13,36,53,0.84))] px-5 py-5 sm:px-7 sm:py-7 lg:px-8 lg:py-8">
              ...
              <div className="relative mt-4 rounded-[26px] border border-white/12 bg-white/12 p-5 backdrop-blur-md sm:p-6 lg:mt-6">
                <div className="grid gap-3 text-slate-100">
                  <div className="flex items-start gap-4">
                    <LazyIcon name="MapPin" className="mt-1 h-5 w-5 shrink-0 text-amber-300" />
                  ...
                  <div className="flex items-center gap-4">
                    <LazyIcon name="Phone" className="h-5 w-5 shrink-0 text-amber-300" />
                    <p className="text-[1rem] font-medium text-slate-100/94">0960 545 6293</p>
                  </div>
                  <div className="flex items-center gap-4">
                    <LazyIcon name="Mail" className="h-5 w-5 shrink-0 text-amber-300" />
          ...
          <section className="bg-[linear-gradient(180deg,rgba(255,255,255,0.88),rgba(248,251,255,0.82))] px-5 py-5 text-slate-900 backdrop-blur-xl sm:px-7 sm:py-7 lg:px-8 lg:py-8">
            <div className="mx-auto flex h-full w-full max-w-[32rem] flex-col justify-center">
              <div>
                ...
                >
                  {providerIsReady ? (
                    <LazyIcon name="ShieldCheck" className="h-4 w-4" />
                  ) : (
              ...
              {showProviderWarning ? (
                <div className="mt-5 rounded-[22px] border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
                  <div className="flex items-start gap-3">
                    <LazyIcon name="AlertTriangle" className="mt-0.5 h-4 w-4 shrink-0" />
              ...
              {isSessionChecking ? (
                <div className="mt-5 rounded-[22px] border border-sky-200 bg-sky-50 px-4 py-3 text-sm text-sky-700">
                  <div className="flex items-center gap-3">
                    <LazyIcon name="LoaderCircle" className="h-4 w-4 animate-spin" />
              ...
              <form onSubmit={handleSubmit} className="mt-5 space-y-4" noValidate>
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <LazyIcon name="Mail" className="pointer-events-none absolute left-5 top-1/2 h-5 w-5 -translate-y-1/2 text-slate-400" />
                    <input
                      id="email"
                      type="email"
                      autoComplete="email"
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <LazyIcon name="LockKeyhole" className="pointer-events-none absolute left-5 top-1/2 h-5 w-5 -translate-y-1/2 text-slate-400" />
                    <input
                      id="password"
                    ...
                    >
                      {showPassword ? <LazyIcon name="EyeOff" size={18} /> : <LazyIcon name="Eye" size={18} />}
                    </button>
                  </div>
                </div>
