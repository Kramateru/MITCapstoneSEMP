export const CALL_SIMULATION_IMPLEMENTATION_PROMPT = `Develop a full-stack Call Simulation mock-call module inside the current codebase and align it to the existing trainer, trainee, KPI, analytics, coaching, reporting, and certificate flows.
...
Primary goal:
Build a production-ready Call Simulation experience for the trainee role where the trainee acts as the CSR, speaks through the mic one turn at a time, waits for the Member actor response between turns, and completes a full mock call that is recorded, transcribed, scored, coached, reported, and stored in Supabase end to end.

Non-negotiable business rules:
- Everything for Call Simulation must be connected to Supabase for create, update, upload, fetch, reporting, and playback workflows.
...
- Every trainee Call Simulation activity must be recorded.
...
- Trainer can create Call Simulation scenarios manually.
- Trainer can modify existing Call Simulation scenarios.
...
- Trainer has a Call Simulation scenario creation, modification, and upload workspace.
...
- Trainer analytics and reports must include Call Simulation performance, retake counts, competency outcomes, and certificate issuance visibility.

Trainee workflow requirements:
- Trainee sees assigned or available Call Simulation scenarios in a scenario dashboard.
- Trainee selects a scenario and enters a pre-call state with status, incoming call cue, and start simulation action.
- Trainee must click the mic icon to start the selected scenario in Call Simulation.
...
- The Member actor then speaks the next script from the scenario uploaded or created in Call Simulation.
...
- After the trainee finishes, trainers can coach the agent and submit whether the trainee is Competent or Not Competent for the selected Call Simulation scenario.
...
- All Call Simulation performance must appear in the trainee performance result area.
- All Call Simulation performance must also feed trainer analytics and reports.
...
- Certificates issued from Call Simulation competency must also appear in trainee reports.
- If a trainer selects a per-trainee report, Call Simulation certificates must be included there as well.
...
Acceptance criteria:
- Trainer can create, modify, upload, publish, and assign Call Simulation scenarios.
...
- Call Simulation performance appears in trainee performance results, trainer analytics, trainer reports, and per-trainee reports.
- Competent outcomes create or expose certificates in certificate navigation and trainee reporting.`

export function getCallSimulationImplementationPrompt() {
  return CALL_SIMULATION_IMPLEMENTATION_PROMPT
}
