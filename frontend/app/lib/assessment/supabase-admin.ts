...
function createSupabaseClientOptions(overrides?: { authorizationToken?: string }) {
  return {
    auth: {
    ...
    global: {
      headers: {
        ...
        ...(overrides?.authorizationToken
          ? { Authorization: `Bearer ${overrides.authorizationToken}` }
          : {}),
      },
    },
  }
...
export function createSupabaseAdminClient() {
  ...
  if (serviceRoleKey) {
    ...
    try {
      return createClient(supabaseUrl, serviceRoleKey, createSupabaseClientOptions())
    } catch (error) {
      console.error('Failed to create Supabase client with service-role configuration:', error)
      throw new Error('Failed to initialize the assessment Supabase client. Please check your Supabase credentials.')
    }
  }
  ...
  if (publicApiKey) {
    ...
    try {
      return createClient(
        supabaseUrl,
        publicApiKey,
        createSupabaseClientOptions({ authorizationToken: requestSupabaseAccessToken || undefined }),
      )
    } catch (error) {
      console.error('Failed to create Supabase client with public key configuration:', error)
      throw new Error('Failed to initialize the assessment Supabase client with the configured public key.')
    }
  }

  console.error('No usable Supabase assessment credentials are configured.', {
    supabaseUrl,
    hasServiceRoleKey: Boolean(serviceRoleKey),
    hasPublicApiKey: Boolean(publicApiKey),
  })
  throw new Error(
    'A valid Supabase service-role key or publishable/anon key is not configured for the assessment workspace. Please set SUPABASE_SERVICE_ROLE_KEY/SUPABASE_SERVICE_KEY or SUPABASE_PUBLISHABLE_KEY/NEXT_PUBLIC_SUPABASE_ANON_KEY.',
  )
}
