...
const AlertCircle = (props: any) => <LazyIcon name="AlertCircle" {...props} />;
...
export default function MyCoaching() {
  ...
  const loadLogs = useCallback(async () => {
    try {
      ...
      const data = await apiFetch<{ logs?: CoachingLog[] }>('/api/certification/coaching/logs', {
        cache: 'no-store',
      });
      ...
      console.error(error);
      setLoadError(error instanceof Error ? error.message : 'Unable to load coaching logs.');
      setCoachingLogs([]);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleAcknowledge = async (logId: string) => {
    try {
      await apiFetch(`/api/certification/coaching/logs/${logId}/acknowledge`, {
        method: 'POST',
      });
      ...
      console.error(error);
      toast.error(error instanceof Error ? error.message : 'Unable to acknowledge coaching log.');
    }
  };
