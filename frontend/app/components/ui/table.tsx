...
function TableHead({ className, ...props }: React.ComponentProps<"th">) {
  return (
    <th
      ...
      className={cn(
        "text-muted-foreground h-10 px-3 text-left align-middle text-[0.8rem] font-semibold uppercase tracking-[0.03em] whitespace-normal sm:h-10 sm:px-4 sm:text-[0.9rem] [&:has([role=checkbox])]:pr-0 [&_[role=checkbox]]:translate-y-[2px]",
        className,
      )}
      {...props}
    />
...
function TableCell({ className, ...props }: React.ComponentProps<"td">) {
  return (
    <td
      ...
      className={cn(
        "px-3 py-3 align-middle whitespace-normal break-words text-sm leading-5 text-foreground sm:px-4 sm:py-3 sm:text-[0.95rem] [&:has([role=checkbox])]:pr-0 [&_[role=checkbox]]:translate-y-[2px]",
        className,
      )}
      {...props}
    />
