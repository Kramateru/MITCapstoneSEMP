      ...
      <div className="grid gap-6 xl:grid-cols-[360px_minmax(0,1fr)]">
          ...
          <CardContent className="space-y-5">
            <div className="flex flex-col items-center text-center">
              ...
              <div className="mt-2 flex items-center gap-2 text-sm text-muted-foreground">
                <LazyIcon name="Mail" className="size-4" />
                {profile?.email}
              </div>
            ...
            <div className="grid gap-3 border-t pt-4 text-sm">
              <div className="flex items-center justify-between gap-4">
                <span className="flex items-center gap-2 text-muted-foreground">
                  <LazyIcon name="CalendarDays" className="size-4" />
                  Created
                </span>
              ...
              <div className="flex items-center justify-between gap-4">
                <span className="flex items-center gap-2 text-muted-foreground">
                  <LazyIcon name="ShieldCheck" className="size-4" />
                  Last Login
                </span>
            ...
            <div className="grid gap-2">
              ...
              <Button type="button" onClick={() => void uploadProfilePicture()} disabled={!selectedFile || uploading}>
                {uploading ? <LazyIcon name="Loader2" className="size-4 animate-spin" /> : <LazyIcon name="Upload" className="size-4" />}
                Upload Picture
              </Button>
              {selectedFile ? (
        ...
        <div className="space-y-6">
          <Card className="border-slate-200 shadow-sm">
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <LazyIcon name="UserRound" className="size-5 text-primary" />
                Change Name
              </CardTitle>
            ...
            <CardContent className="space-y-4">
              ...
              <Button type="button" onClick={() => void saveName()} disabled={savingName || Boolean(nameError)}>
                {savingName ? <LazyIcon name="Loader2" className="size-4 animate-spin" /> : <LazyIcon name="Save" className="size-4" />}
                Save Name
              </Button>
            </CardContent>
          </Card>
          ...
          <Card className="border-slate-200 shadow-sm">
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <LazyIcon name="KeyRound" className="size-5 text-primary" />
                Change Password
              </CardTitle>
            ...
            <CardContent className="space-y-5">
                ...
                <div className="grid gap-2 text-sm text-muted-foreground sm:grid-cols-2">
                  {[
                    ...
                    <span key={String(label)} className="flex items-center gap-2">
                      <LazyIcon name="CheckCircle2" className={`size-4 ${passed ? 'text-emerald-600' : 'text-slate-300'}`} />
                      {label}
                    </span>
                  ))}
                </div>
              ...
              <Button type="button" onClick={() => void changePassword()} disabled={!canChangePassword || changingPassword}>
                {changingPassword ? <LazyIcon name="Loader2" className="size-4 animate-spin" /> : <LazyIcon name="KeyRound" className="size-4" />}
                Update Password
              </Button>
            </CardContent>
          </Card>
