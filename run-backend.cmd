...
if not defined BACKEND_REQUIREMENTS_INSTALL set "BACKEND_REQUIREMENTS_INSTALL=1"
if not defined BACKEND_REQUIREMENTS_INSTALL_ARGS set "BACKEND_REQUIREMENTS_INSTALL_ARGS=--upgrade"
...
if not exist "%PYTHON_EXE%" for /f "delims=" %%I in ('where python.exe 2^>nul') do set "PYTHON_EXE=%%I"
if not exist "%PYTHON_EXE%" for /f "delims=" %%I in ('where py.exe 2^>nul') do set "PYTHON_EXE=%%I" & set "PYTHON_ARGS=-3"

if not exist "%PYTHON_EXE%" (
  echo Python runtime not found.
  echo Install Python 3 and ensure "python.exe" or "py.exe" is on PATH before starting the backend.
  exit /b 1
)

if /I "%BACKEND_REQUIREMENTS_INSTALL%"=="1" (
  echo Syncing backend Python dependencies to the latest pinned state...
  if defined PYTHON_ARGS (
    "%PYTHON_EXE%" %PYTHON_ARGS% -m pip install %BACKEND_REQUIREMENTS_INSTALL_ARGS% -r "%BACKEND_DIR%\requirements.txt"
  ) else (
    "%PYTHON_EXE%" -m pip install %BACKEND_REQUIREMENTS_INSTALL_ARGS% -r "%BACKEND_DIR%\requirements.txt"
  )
...
if /I not "%BACKEND_PRESTART_COMPILE%"=="0" (
  echo Verifying backend Python files before startup...
  if defined PYTHON_ARGS (
    "%PYTHON_EXE%" %PYTHON_ARGS% -m compileall -q "%BACKEND_DIR%"
  ) else (
...
echo Starting backend in Supabase production mode using the latest checked-in code...
echo The backend will validate Supabase and sync local users into auth.users during startup.
echo Backend URL: %BACKEND_URL%

if defined PYTHON_ARGS (
  "%PYTHON_EXE%" %PYTHON_ARGS% -m uvicorn backend.main:app --host %BACKEND_HOST% --port %BACKEND_PORT%
) else (
