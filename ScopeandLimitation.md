...
### 7.5 Startup Script Limitations and Overrides

The repository includes root-level launcher scripts for the current production-style workflow:

- `run-backend.cmd`
- `run-frontend.cmd`

These scripts are intended to run from the repository root and they load environment values from both the root and service-specific `.env` files.

Key launch-time behaviors:

- `run-backend.cmd` will install Python dependencies by default and verify backend source files before startup.
- `run-frontend.cmd` will install frontend dependencies by default, clear stale `.next` artifacts, build the app, and then start the production frontend server.
- Both scripts support automatic listener restarts when the configured port is already in use.

Important environment variable overrides:

- `BACKEND_REQUIREMENTS_INSTALL=0` — skip backend pip dependency sync
- `BACKEND_REQUIREMENTS_INSTALL_ARGS` — additional pip install flags for backend dependency sync (default: `--upgrade`)
- `BACKEND_PRESTART_COMPILE=0` — skip backend compile-time verification
- `FRONTEND_INSTALL_DEPS=0` — skip frontend dependency installation
- `FRONTEND_INSTALL_DEPS_LEGACY_PEER_DEPS=1` — force `npm install --legacy-peer-deps` for frontend dependency resolution
- `FRONTEND_NPM_INSTALL_ARGS` — additional arguments for frontend dependency installation (default: `--no-audit --no-fund`)
- `SKIP_FRONTEND_BUILD=1` — skip the Next.js build step when a valid `.next` build already exists
- `RESTART_IF_RUNNING=1` — stop an existing listener on the configured port if it is already running
- `STRICT_SUPABASE_PROJECT_CHECK=1` — fail startup when Supabase URL and keys point to different projects

Startup limitations to keep in mind:

- The frontend installation step may require `FRONTEND_INSTALL_DEPS_LEGACY_PEER_DEPS=1` for the current React 19 / `react-day-picker` dependency combination.
- `SKIP_FRONTEND_BUILD=1` only works if a compiled frontend build already exists in `.next`.
...
- Critical media uploads (profile images, microlearning audio, call simulation assets) are intended to use Supabase storage only; local fallback behavior is not supported in production.
...
In production, `ALLOW_LOCAL_MEDIA_FALLBACK` should be set to `false`, and media upload flows are expected to fail if Supabase storage is unavailable.
