...
if not defined FRONTEND_INSTALL_DEPS set "FRONTEND_INSTALL_DEPS=1"
if not defined FRONTEND_INSTALL_DEPS_LEGACY_PEER_DEPS set "FRONTEND_INSTALL_DEPS_LEGACY_PEER_DEPS=1"
if not defined FRONTEND_NPM_INSTALL_ARGS set "FRONTEND_NPM_INSTALL_ARGS=--no-audit --no-fund"
...
set "NPM_CMD="
set "NPM_CMD_IS_BATCH=0"
for /f "delims=" %%I in ('where npm.cmd 2^>nul') do if not defined NPM_CMD (
  set "NPM_CMD=%%I"
  set "NPM_CMD_IS_BATCH=1"
)
if not defined NPM_CMD if exist "npm.cmd" (
  set "NPM_CMD=npm.cmd"
  set "NPM_CMD_IS_BATCH=1"
)
if not defined NPM_CMD set "NPM_CMD=npm"

if /I "%FRONTEND_INSTALL_DEPS%"=="1" (
  echo Syncing frontend dependencies to the latest checked-in state...
  if /I "%FRONTEND_INSTALL_DEPS_LEGACY_PEER_DEPS%"=="1" (
    set "NPM_INSTALL_ARGS=%FRONTEND_NPM_INSTALL_ARGS% --legacy-peer-deps"
  ) else (
    set "NPM_INSTALL_ARGS=%FRONTEND_NPM_INSTALL_ARGS%"
  )
  if /I "%NPM_CMD_IS_BATCH%"=="1" (
    call "%NPM_CMD%" install %NPM_INSTALL_ARGS%
  ) else (
    "%NPM_CMD%" install %NPM_INSTALL_ARGS%
  )
  if errorlevel 1 exit /b 1
)

if not exist "node_modules" (
  echo Frontend dependencies are still missing after npm install.
  exit /b 1
)
...
echo Starting frontend in production mode using the latest checked-in build...
if /I "%NPM_CMD_IS_BATCH%"=="1" (
  call "%NPM_CMD%" run start -- --hostname %FRONTEND_HOST% --port %FRONTEND_PORT%
) else (
  "%NPM_CMD%" run start -- --hostname %FRONTEND_HOST% --port %FRONTEND_PORT%
)
exit /b %errorlevel%
