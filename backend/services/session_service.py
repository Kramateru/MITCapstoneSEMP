...
def get_single_session_mode() -> str:
    """Return the single-session policy.

    Default to the more forgiving replace mode when the deployment does not
    explicitly set SINGLE_SESSION_MODE. This avoids stale session conflicts
    during normal login flows while still allowing an explicit "block" policy
    to be enforced in environments that opt into it.
    """
    raw_mode = os.getenv("SINGLE_SESSION_MODE")
    if raw_mode is None:
        return "replace"
