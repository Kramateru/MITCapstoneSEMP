...
class SupabaseClient:
        ...
        *,
        ...
        if public_url:
            if save_local_backup:
                logger.info(
                    "Ignoring local backup request for Call Simulation audio because recordings must remain Supabase-only: %s",
                    relative_path,
                )
            logger.info(f"Call Simulation audio uploaded: {path}")
            return public_url

        logger.error("Failed to upload Call Simulation audio file to Supabase.")
        return None
    ...
    def upload_call_simulation_asset(
        ...
        *,
        ...
        logger.error("Failed to upload Call Simulation asset to Supabase.")
        return None
    ...
    def upload_profile_image(
        ...
        Profile images are required to be stored in Supabase for production
        environments and do not fall back to local media storage.
        """
        path = f"profiles/{user_id}/{filename}"

        if not self.is_available:
            logger.error(
                "Supabase storage is unavailable. Profile image upload rejected: %s",
                path,
            )
            return None
    ...
    def upload_microlearning_audio(
        ...
        if allow_local_fallback and self._allow_local_media_fallback():
            logger.warning(
                "Supabase upload failed or is unavailable. Using local media fallback for microlearning audio: %s",
                relative_path,
            )
        ...
        logger.error("Microlearning audio upload failed and local fallback is disabled: %s", relative_path)
        return None
