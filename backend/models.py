...
class CallSimulationEvent(Base):
    """Auditable Call Simulation event log for call-flow actions and recovery events."""

    __tablename__ = "call_simulation_event"

    id = Column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    session_id = Column(String(36), ForeignKey("sim_session.id"), nullable=False, index=True)
    scenario_id = Column(String(36), ForeignKey("scenario.id"), nullable=True, index=True)
    user_id = Column(String(36), ForeignKey("user.id"), nullable=False, index=True)
    ...
    session = relationship("SimSession", backref="call_events")
    scenario = relationship("Scenario")
